...
export default function FeedbackDetailPage() {
  ...
  const handleUpvote = async () => {
    if (!feedback || upvoteLoading) return;
    try {
      setUpvoteLoading(true);
      const response = await feedbackAPI.upvote(feedback._id);
      setFeedback(response.data);
      toast.success(response.data.upvotedBy.some((id: string) => id === user?._id) ? 'Upvoted!' : 'Upvote removed');
    } catch (error: any) {
  ...
  const hasUpvoted = feedback.upvotedBy.some((id: string) => id === user?._id);
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 mb-6">
          <div className="flex items-start justify-between gap-4 mb-4">
            ...
            <button
              ...
              } ${upvoteLoading ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
              title={hasUpvoted ? 'Click to remove your upvote' : 'Click to upvote'}
            >
              {upvoteLoading ? (
