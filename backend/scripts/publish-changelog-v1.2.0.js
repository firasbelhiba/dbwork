...
async function publishChangelog() {
  try {
    ...
    // Find v1.2.0 changelog
    const changelog = await Changelog.findOne({ version: '1.2.0' }).exec();

    if (!changelog) {
      console.error('Changelog v1.2.0 not found!');
      process.exit(1);
    }

    if (changelog.isPublished) {
      console.log('Changelog v1.2.0 is already published!');
      process.exit(0);
    }
    ...
    console.log('\n✅ Changelog v1.2.0 published successfully!');
