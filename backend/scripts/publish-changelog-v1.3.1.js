...
async function publishChangelog() {
  try {
    ...
    // Find v1.3.1 changelog
    const changelog = await Changelog.findOne({ version: '1.3.1' }).exec();

    if (!changelog) {
      console.error('Changelog v1.3.1 not found!');
      console.error('Please run add-changelog-v1.3.1.js first.');
      process.exit(1);
    }

    if (changelog.isPublished) {
      console.log('Changelog v1.3.1 is already published!');
      process.exit(0);
    }
    ...
    console.log('\n✅ Changelog v1.3.1 published successfully!');
